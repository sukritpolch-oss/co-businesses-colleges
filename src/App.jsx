...
// --- 🟢 เพิ่มข้อมูลรายชื่อวิทยาลัยตรงนี้ 🟢 ---
const VOCATIONAL_COLLEGES = [
  "วิทยาลัยเทคนิคเชียงใหม่", "วิทยาลัยอาชีวศึกษาเชียงใหม่", "วิทยาลัยสารพัดช่างเชียงใหม่", "วิทยาลัยเกษตรและเทคโนโลยีเชียงใหม่",
  "วิทยาลัยเทคนิคเชียงราย", "วิทยาลัยอาชีวศึกษาเชียงราย", "วิทยาลัยการอาชีพเชียงราย", "วิทยาลัยเกษตรและเทคโนโลยีเชียงราย",
  "วิทยาลัยเทคนิคลำปาง", "วิทยาลัยอาชีวศึกษาลำปาง", "วิทยาลัยการอาชีพเกาะคา", "วิทยาลัยการอาชีพเถิน",
  "วิทยาลัยเทคนิคลำพูน", "วิทยาลัยเกษตรและเทคโนโลยีลำพูน", "วิทยาลัยการอาชีพป่าซาง",
  "วิทยาลัยเทคนิคแพร่", "วิทยาลัยอาชีวศึกษาแพร่", "วิทยาลัยเกษตรและเทคโนโลยีแพร่",
  "วิทยาลัยเทคนิคน่าน", "วิทยาลัยสารพัดช่างน่าน", "วิทยาลัยการอาชีพเวียงสา",
  "วิทยาลัยเทคนิคพะเยา", "วิทยาลัยเกษตรและเทคโนโลยีพะเยา", "วิทยาลัยการอาชีพเชียงคำ",
  "วิทยาลัยเทคนิคแม่ฮ่องสอน", "วิทยาลัยการอาชีพแม่สะเรียง", "วิทยาลัยการอาชีพนวมินทราชินีแม่ฮ่องสอน",
  "วิทยาลัยเทคนิคตาก", "วิทยาลัยอาชีวศึกษาตาก", "วิทยาลัยเกษตรและเทคโนโลยีตาก", "วิทยาลัยการอาชีพแม่สอด",
  "วิทยาลัยเทคนิคกำแพงเพชร", "วิทยาลัยอาชีวศึกษากำแพงเพชร", "วิทยาลัยเกษตรและเทคโนโลยีกำแพงเพชร",
  "วิทยาลัยเทคนิคพิษณุโลก", "วิทยาลัยอาชีวศึกษาพิษณุโลก", "วิทยาลัยพณิชยการบึงพระพิษณุโลก",
  "วิทยาลัยเทคนิคนครสวรรค์", "วิทยาลัยอาชีวศึกษานครสวรรค์", "วิทยาลัยการอาชีพบรรพตพิสัย"
];
const App = () => {
  // --- ระบบ Login & Security ---
...
// --- VEC Hub Logic ---
  ...
  const shareToVecHub = async () => {
    ...
    showStatus("กำลังแชร์เข้าคลังแผนฝึกอาชีวศึกษา...");
    try {
      const safeConfig = { ...config, userApiKey: '', openaiApiKey: '', claudeApiKey: '', deepseekApiKey: '' };
      ...
      showStatus("แชร์ข้อมูลเข้าคลังอาชีวศึกษาสำเร็จ!");
      setActiveTab('vechub');
    } catch (err) {
      showStatus("เกิดข้อผิดพลาดในการเชื่อมต่อ กรุณาลองอีกครั้ง");
    }
                      ...
                      {f.k === 'level' ? (
                        ...
                        <>
                          <input
                            ...
                            // 🟢 1. กำหนด list id ให้ input เฉพาะคอลัมน์ชื่อวิทยาลัย 🟢
                            list={f.k === 'collegeName' ? 'college-list' : undefined}
                          />
                          
                          // 🟢 2. สร้าง datalist ผูกไว้ด้านล่าง 🟢
                          {f.k === 'collegeName' && (
                            <datalist id="college-list">
                              {VOCATIONAL_COLLEGES.map(college => (
          ...
          <div className="max-w-6xl mx-auto space-y-8 animate-in slide-in-from-bottom-5 duration-500 font-serif">
            <div className="bg-indigo-600 p-8 rounded-3xl text-white shadow-xl flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <div className="flex flex-col gap-3 w-full md:w-auto">
                ...
                {!editingCloudId && (
                  <button onClick={shareToVecHub} className="bg-emerald-500 text-white px-8 py-4 rounded-2xl font-black text-sm hover:bg-emerald-600 shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2">
                    <Library size={20} /> แชร์เข้าคลังของวิทยาลัย
                  </button>
                )}
              </div>
            </div>
                ...
                {vecHubData.length === 0 ? (
                  ...
                  vecHubData.filter(item => 
                    ...
                    <div key={item.id || idx} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 hover:border-emerald-300 transition-colors flex flex-col h-full">
                      <div className="flex-1">
                        <h3 className="font-black text-emerald-900 text-lg leading-tight mb-2">{item.company || 'ไม่ระบุชื่อบริษัท'}</h3>
                        ...
                        <button
                          onClick={() => {
                            try {
                              ...
                              loadFromCloudItem({
                                ...parsedData,
                                config: { ...parsedData.config, collegeName: item.college, companyName: item.company, province: item.province }
                              });
                            } catch(e) { showStatus("เกิดข้อผิดพลาดในการโหลดข้อมูล"); }
                          }}
                          className="w-full bg-emerald-100 hover:bg-emerald-200 text-emerald-700 py-2 rounded-xl text-xs font-black transition-colors flex justify-center items-center gap-1"
                        >
                          <Wand2 size={14} /> นำเข้าแผนฝึกนี้
                        </button>
                      </div>
                    </div>
                  ))
