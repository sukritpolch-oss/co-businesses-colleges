...
const App = () => {
  ...
  const handleVecHubImport = (item) => {
    try {
      const parsedData = JSON.parse(item.dataPayload);

      // ผสานข้อมูลพื้นฐานจาก VEC Hub เข้าไปใน config ป้องกันข้อมูลแหว่ง
      if (parsedData.config) {
        parsedData.config.collegeName = item.college || parsedData.config.collegeName;
        parsedData.config.companyName = item.company || parsed.config.companyName;
        parsedData.config.province = item.province || parsed.config.province;
      }

      // ตรวจสอบว่าหน้าจอปัจจุบันมีข้อมูลค้างอยู่หรือไม่
      ...
      if (currentHasData) {
        // ถ้ามีข้อมูลอยู่แล้ว ให้แสดงหน้าต่างเตือน (ตัวเดียวกับตอนอัปโหลดไฟล์ Local) เพื่อให้เลือกว่าจะ รวมงาน หรือ เขียนทับ
        setPendingDveData(parsedData);
        setShowDveConflictModal(true);
      } else {
        // ถ้าหน้าจอว่างเปล่า ให้เขียนทับ (โหลดข้อมูลทั้งหมด) ทันที
        executeApplyDveData(parsedData, 'overwrite');
        setActiveTab('setup'); // พาไปหน้าตั้งค่าเพื่อดูข้อมูลรวม
      }
    } catch (e) {
  ...
  const shareToVecHub = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        workplaceMainTasks,
        selectedBehaviors,
        subjects: cleanedSubjects // 🟢 ส่งข้อมูลวิชาทั้งหมดเข้าไปบันทึกด้วย
      };
