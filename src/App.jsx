...
const COLLEGE_HUB_API_URL = "https://script.google.com/macros/s/AKfycbyPLA8syhsnL0KuaEM00LeQH8zQnhKwMTtSVUCPtxWmSMipZUgnAepkLvK-yPRD0OxgSQ/exec";
// ... ภายใน const App = () => { ...
const [collegeHubData, setCollegeHubData] = useState([]);
const [hubFilters, setHubFilters] = useState({ major: '', company: '', province: '' });
...
const App = () => {
  ...
  // ดึงข้อมูลคลังแผนฝึกวิทยาลัย
  const fetchCollegeHubData = async () => {
    if (!COLLEGE_HUB_API_URL) return;
    setIsHubLoading(true);
    try {
      const res = await fetch(COLLEGE_HUB_API_URL, {
        method: 'POST', body: JSON.stringify({ action: 'getPlans' }), headers: { 'Content-Type': 'text/plain;charset=utf-8' }
      });
      const result = await res.json();
      if (result.status === 'success') {
        // กรองข้อมูลให้เห็นเฉพาะวิทยาลัยของตนเอง (แอดมินเห็นทั้งหมด)
        const myCollegeData = result.data.filter(plan =>
          isDeveloper || plan.collegeName === config.collegeName
        );
        setCollegeHubData(myCollegeData);
      }
    } catch (e) {
      console.error(e);
      showStatus("เชื่อมต่อคลังวิทยาลัยไม่สำเร็จ");
    }
    setIsHubLoading(false);
  };

  // ดึงข้อมูลเมื่อเข้าแท็บ
  useEffect(() => {
    if (activeTab === 'college_hub' && isAuthenticated) {
      fetchCollegeHubData();
    }
  }, [activeTab, isAuthenticated, config.collegeName]);
      
  // ฟังก์ชันแชร์ลงคลังวิทยาลัย
  const shareToCollegeHub = async () => {
    if (!config.companyName?.trim()) return showStatus("กรุณาระบุชื่อสถานประกอบการ");
    if (!config.major?.trim()) return showStatus("กรุณาระบุสาขาวิชาในหน้าตั้งค่า");

    showStatus("กำลังแชร์ข้อมูลลงคลังวิทยาลัย...");
    try {
      const payload = { config, workplaceMainTasks, subjects: subjects.filter(s => s.isAnalyzed) };
      await fetch(COLLEGE_HUB_API_URL, {
        method: 'POST',
        body: JSON.stringify({
          action: 'sharePlan',
      ...
      showStatus("แชร์เข้าคลังวิทยาลัยสำเร็จ!");
    } catch (e) {
      showStatus("เกิดข้อผิดพลาดในการแชร์");
    }
  };

  // ฟังก์ชันแจ้งลบ/แก้ไข (สำหรับเจ้าของ)
  const requestActionCollegeHub = async (id, actionType) => {
    if (!window.confirm(`ยืนยันการแจ้งขอ${actionType === 'delete' ? 'ลบ' : 'แก้ไข'}แผนนี้?`)) return;
    showStatus("กำลังส่งคำขอไปยังแอดมิน...");
    try {
      await fetch(COLLEGE_HUB_API_URL, {
        method: 'POST',
        body: JSON.stringify({ action: 'requestAction', id, requestType: actionType, creatorName: config.trainerName, creatorEmail: currentUserEmail, companyName: config.companyName }),
        headers: { 'Content-Type': 'text/plain;charset=utf-8' }
      });
      showStatus(`ส่งคำขอ${actionType === 'delete' ? 'ลบ' : 'แก้ไข'}เรียบร้อย แอดมินจะแจ้งเตือนทางอีเมลเมื่อดำเนินการเสร็จ`);
      fetchCollegeHubData();
    } catch (e) {
      showStatus("ส่งคำขอไม่สำเร็จ");
    }
  };

  // ฟังก์ชันอนุมัติ (สำหรับ Admin)
  const adminApproveCollegeHub = async (id, approveType) => {
    showStatus(`กำลังดำเนินการ${approveType === 'delete' ? 'ลบ' : 'อนุมัติแก้ไข'}...`);
    try {
      await fetch(COLLEGE_HUB_API_URL, {
        method: 'POST',
        body: JSON.stringify({ action: 'adminApprove', id, approveType }),
        headers: { 'Content-Type': 'text/plain;charset=utf-8' }
      });
      showStatus(`ดำเนินการเสร็จสิ้น ระบบส่งอีเมลแจ้งเตือนผู้แชร์แล้ว`);
      fetchCollegeHubData();
    } catch (e) {
      showStatus("ดำเนินการไม่สำเร็จ");
    }
  };
...
- **กฎเหล็ก**: ต้องตอบข้อมูลเป็น **ภาษาไทยเท่านั้น** กระชับและตรงประเด็น **ห้าม** อธิบายกระบวนการคิด หรือเขียนเรียงความภาษาอังกฤษ (เช่น ห้ามมีคำว่า Mapping: ...) ลงมาในเนื้อหาเด็ดขาด`;
  ...
  const navItems = [
    ...
    { id: 'college_hub', baseLabel: 'คลังแผนฝึก(วิทยาลัย)', i: LayoutDashboard },
                  ...
                  <ul className="text-xs space-y-1 list-disc list-inside pl-4 text-indigo-100">
                    ...
                    <div className="flex flex-col gap-3 w-full md:w-auto">
                      <button
                        ...
                        className="bg-white text-indigo-600 px-8 py-3 rounded-2xl font-black text-sm hover:bg-indigo-50 shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2"
                      >
                        <Cloud size={20} /> แชร์เข้าคลังส่วนกลาง (ทุกวิทยาลัย)
                      </button>
                      <button
                        onClick={shareToCollegeHub}
                        className="bg-emerald-500 text-white px-8 py-3 rounded-2xl font-black text-sm hover:bg-emerald-600 shadow-lg border border-emerald-400 active:scale-95 transition-all flex items-center justify-center gap-2"
                      >
                        <Building2 size={20} /> แชร์เข้าคลังวิทยาลัย (เฉพาะวิทยาลัยตนเอง)
                      </button>
                    </div>
                  </ul>
                </div>
            ...
            <div className="max-w-6xl mx-auto space-y-8 animate-in slide-in-from-bottom-5 duration-500 font-serif">
              <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm font-serif">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {/* เพิ่มเงื่อนไขเช็คว่าถ้ายังไม่ได้เลือกจังหวัด และ ยังไม่ได้พิมพ์ค้นหา ให้แสดงข้อความ */}
                  {(!filterProvince && searchQuery.trim() === '') ? (
                    ...
                    (() => {
                      ...
                      // ถ้าค้นหาแล้วไม่เจออะไรเลย ให้แสดงข้อความว่าไม่พบข้อมูล
                      ...
                      // ถ้าเจอข้อมูล ให้แสดงการ์ดตามปกติ
                      return filteredData.map((item, idx) => (
                        <div key={item.id || idx} className={`bg-slate-50 border rounded-2xl p-5 hover:shadow-md transition-shadow group flex flex-col h-full relative ${item.deleteRequest ? 'border-orange-300 bg-orange-50/50' : 'border-slate-200'}`}>
                          <div className="flex-1">
                            ...
                            {/* --- ส่วนที่เพิ่มใหม่: กล่องแสดงชื่องานหลัก --- */}
      ...
      <div className="max-w-6xl mx-auto space-y-8 animate-in slide-in-from-bottom-5 duration-500 font-serif">
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm font-serif">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-slate-100 pb-6">
            <div>
              <h2 className="text-xl font-black text-slate-800 flex items-center gap-2">
                <LayoutDashboard className="text-emerald-500" /> คลังแผนฝึกอาชีพอาชีวศึกษา
                {isDeveloper && <span className="ml-2 bg-amber-100 text-amber-700 text-[10px] px-2 py-1 rounded-md">โหมดผู้ดูแลระบบ</span>}
              </h2>
              <p className="text-sm text-slate-500 mt-1">
                ข้อมูลเฉพาะของวิทยาลัย: <span className="font-bold text-indigo-600">{config.collegeName || 'ยังไม่ได้ระบุวิทยาลัย'}</span>
              </p>
            </div>
            {/* ตัวกรอง */}
            <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
              <input
                type="text" placeholder="ค้นหาสาขาวิชา..."
                className="px-4 py-2 border rounded-xl text-sm"
                value={hubFilters.major} onChange={e => setHubFilters({ ...hubFilters, major: e.target.value })}
              />
              <input
                type="text" placeholder="สถานประกอบการ..."
                className="px-4 py-2 border rounded-xl text-sm"
                value={hubFilters.company} onChange={e => setHubFilters({ ...hubFilters, company: e.target.value })}
              />
              <select
                className="px-4 py-2 border rounded-xl text-sm"
                value={hubFilters.province} onChange={e => setHubFilters({ ...hubFilters, province: e.target.value })}
              >
                <option value="">ทุกจังหวัด</option>
                {PROVINCES.map(p => <option key={`h-${p}`} value={p}>{p}</option>)}
              </select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {isHubLoading ? (
              <div className="col-span-full py-16 text-center text-slate-400"><Loader2 className="animate-spin inline mr-2" /> กำลังโหลด...</div>
            ) : (
              (() => {
                const filtered = collegeHubData.filter(item => {
                  return (hubFilters.major === '' || item.major?.includes(hubFilters.major)) &&
                    (hubFilters.company === '' || item.companyName?.includes(hubFilters.company)) &&
                    (hubFilters.province === '' || item.province === hubFilters.province);

                });

                if (filtered.length === 0) return <div className="col-span-full py-10 text-center">ไม่พบข้อมูลแผนฝึกในวิทยาลัยของคุณ</div>;

                return filtered.map((item, idx) => {
                  // ตรวจสอบความเป็นเจ้าของแผน (เช็คผ่านอีเมล)
                  const isOwner = currentUserEmail.toLowerCase() === (item.creatorEmail || '').toLowerCase();

                  return (
                    <div key={idx} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 hover:shadow-md transition-shadow relative">
                      <h3 className="font-black text-indigo-900 text-lg mb-2">{item.companyName}</h3>
                      <div className="space-y-1 mb-4 text-xs font-bold text-slate-600">
                        <p>สาขาวิชา: <span className="text-slate-800">{item.major || '-'}</span></p>
                        <p>จังหวัด: {item.province || '-'}</p>
                        <p>แชร์โดย: <span className="text-blue-600">{item.creatorName}</span></p>
                        {item.status.includes('pending') && (
                          <p className="text-orange-600 mt-2">สถานะ: รอแอดมินอนุมัติ({item.status === 'pending_delete' ? 'ลบ' : 'แก้ไข'})</p>
                        )}
                      </div>
                      <div className="flex flex-wrap gap-2 pt-4 border-t border-slate-200 mt-auto">
                        {/* ปุ่มสำหรับทุกคนในวิทยาลัย */}
                        <button onClick={() => executeApplyDveData(item.dataPayload, 'overwrite')} className="flex-1 bg-indigo-100 text-indigo-700 py-1.5 rounded-lg text-xs font-bold hover:bg-indigo-200">
                          นำเข้าใช้งาน
                        </button>

                        {/* การจัดการสำหรับเจ้าของแผน (แก้ไข / แจ้งลบ) */}
                        {isOwner && !isDeveloper && !item.status.includes('pending') && (
                          <>
                            <button onClick={() => requestActionCollegeHub(item.id, 'edit')} className="bg-amber-100 text-amber-700 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-amber-200">
                              แจ้งแก้ไข
                            </button>
                            <button onClick={() => requestActionCollegeHub(item.id, 'delete')} className="bg-red-100 text-red-700 px-3 py-1.5 rounded-lg text-[10px] font-bold hover:bg-red-200">
                              แจ้งลบ
                            </button>
                          </>
                        )}

                        {/* การจัดการสำหรับแอดมิน */}
                        {isDeveloper && item.status.includes('pending') && (
                          <div className="w-full flex gap-2 mt-2">
                            <button onClick={() => adminApproveCollegeHub(item.id, item.status.replace('pending_', ''))} className="flex-1 bg-green-500 text-white py-1.5 rounded-lg text-[10px] font-bold hover:bg-green-600">
                              อนุมัติ{item.status === 'pending_delete' ? 'ลบ' : 'แก้ไข'}
                            </button>
                          </div>
                        )}
                      </div>
