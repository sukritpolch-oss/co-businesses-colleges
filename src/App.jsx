...
const App = () => {
  ...
  const getCourseLibraryItemKey = (item, index = 0) => {
    return (
      item.id ||
      `${item.college || ''}-${item.department || ''}-${item.subject?.code || ''}-${item.subject?.name || ''}-${index}`
    );
  };
  const importSharedCourse = (item) => {
    if (!item?.subject) {
      return showStatus('ไม่พบข้อมูลรายวิชา');
    }

    const importedSubject = normalizeSharedSubject(item.subject);
  ...
  const importSelectedSharedCourses = () => {
    const selectedItems = courseLibraryData.filter((item, index) =>
      selectedCourseLibraryIds.includes(getCourseLibraryItemKey(item, index))
    );
                    ...
                    return (
                      <section key={groupKey}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                          {items.map((item, index) => {
                            const itemKey = getCourseLibraryItemKey(item, index);
                            const isOwner =
                              ...
                              >
                                <div className="flex justify-between gap-3">
                                  <div className="flex items-start gap-3">
                                    <input
                                      type="checkbox"
                                      checked={selectedCourseLibraryIds.includes(itemKey)}
                                  ...
                                  {canDelete && (
                                    <button
                                      onClick={() => deleteSharedCourse(item)}
                                ...
                                <button
                                  onClick={() => importSharedCourse(item)}
