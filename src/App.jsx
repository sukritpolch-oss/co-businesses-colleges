...
const App = () => {
  ...
  const cleanTaskName = (name) => {
    if (!name) return '';
    // ดักจับและแปลง Object ให้เป็น String เสมอ ป้องกันจอขาวจาก .replace()
    let strName = typeof name === 'object' ? JSON.stringify(name) : String(name);
    let cleaned = strName.replace(/ศึกษา|เรียนรู้|ทฤษฎี/g, '').trim();
