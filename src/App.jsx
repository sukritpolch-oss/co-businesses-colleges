...
const App = () => {
  ...
  const runSubjectAnalysis = async (sub) => {
    ...
    2. **สำคัญมากมาก**: ต้องวิเคราะห์สมรรถนะรายวิชา คำอธิบายรายวิชา จุดประสงค์รายวิชา และผลลัพธ์การเรียนรู้ เพื่อสร้าง "งานหลัก" และ "งานย่อย" ให้ครบถ้วนเชิงปฏิบัติ
   - ต้องสร้างงานหลักอย่างน้อย 3-5 งานต่อรายวิชา
   - งานหลักแต่ละงานต้องมีงานย่อยอย่างน้อย 3-5 งาน
   - ห้ามตอบสั้น ห้ามตอบเพียงหัวข้อกว้าง ๆ
   - งานย่อยต้องเป็นงานที่ผู้เรียนลงมือทำได้จริงในสถานประกอบการ
   - ต้องครอบคลุมงานตั้งแต่ การเตรียมงาน การปฏิบัติงาน การตรวจสอบคุณภาพ การแก้ไขข้อบกพร่อง การจัดเก็บ/บันทึก/ส่งมอบงาน

3. รหัสงานต้องขึ้นต้นด้วย "${sub.id}" เสมอ
   - งานหลักใช้รหัส เช่น ${sub.id}1, ${sub.id}2, ${sub.id}3
   - งานย่อยใช้รหัส เช่น ${sub.id}1-1, ${sub.id}1-2, ${sub.id}2-1
   - ห้ามข้ามลำดับรหัส
   - ห้ามสร้างรหัสที่ไม่สัมพันธ์กับรายวิชา
                ...
                <div id="dve-0405-area" className={`Section2 ${activeReportView !== 'dve0405' ? 'hidden' : ''}`}>
                  <div className="page-break mb-20 font-serif border-t-2 pt-10 border-dashed border-slate-300">
                    ...
                    <div className="report-header font-serif text-[11pt] space-y-1.5">
                      ...
                      <p>ฝึกงานปีการศึกษา {config.academicYear || '.........'} ระหว่างวันที่ {config.startDate || '.........'} ถึง วันที่ {config.endDate || '.........'} เวลาฝึก {totalAssignedHours > 0 ? Number((totalAssignedHours / (config.hoursPerDay || 8)).toFixed(1)) : 0} วัน / {totalAssignedHours} ชั่วโมง</p>                      {!isWorkplaceTrainer && <p>ผลลัพธ์การเรียนรู้ (ของวิชาที่นำไปฝึกในสถานประกอบการ): <span className="font-bold">{analyzedSubjectNames || '................'}</span></p>}
                    </div>
