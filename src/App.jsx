...
const App = () => {
  ...
  const remapAllWorkplaceTasks = async () => {
    ...
    setIsRemapping(true); showStatus("กำลังส่งข้อมูลให้ AI วิเคราะห์เพื่อจับคู่งานใหม่...");
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center font-serif p-4 relative overflow-hidden">
        <div className="absolute top-[-10%] left-[-10%] w-96 h-96 bg-indigo-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-96 h-96 bg-blue-400 rounded-full mix-blend-multiply filter blur-3xl opacity-20"></div>
  ...
  return (
    <div className={`min-h-screen font-sans text-slate-900 pb-20 font-serif transition-colors duration-500 ${activeTab === 'cloud' ? 'bg-slate-800' : 'bg-slate-50'}`}>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 h-16 flex items-center justify-around z-50 shadow-2xl font-serif px-2 overflow-x-auto overflow-y-hidden whitespace-nowrap hide-scrollbar">
        {navItems.map(nav => (
          <button key={nav.id} onClick={() => nav.href ? window.open(nav.href, '_blank', 'noopener,noreferrer') : setActiveTab(nav.id)} className={`flex flex-col items-center justify-center gap-1 w-16 flex-shrink-0 h-full px-1 ${activeTab === nav.id ? 'text-indigo-600 border-t-2 border-indigo-600 -mt-[1px]' : 'text-slate-400'}`}>
